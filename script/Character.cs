...
public class CCharacter : CEntity
{
    ...
    public class CState
    {
        ...
        public EState CurrentState
        {
            ...
            set
            {
                ...
                {
                    CLogManager.LogWarning($"{m_currentState}无法转换到{value}状态");
                }
            }
        }
        public CState()
    ...
    public CState.EState State
    {
        ...
        set
        {
            ...
            CLogManager.LogInfo($"{Name}由{m_state.CurrentState}状态转移到{value}状态");
    ...
    public bool MoveOn(Vector3Int target)
    {
        ...
        {
            CLogManager.LogError($"{Name}的m_APRepo没有{EAction.Movement}属性");
            return false;
        }
        if (dist > rest_movement)
        {
            CLogManager.LogError($"{Name}超出移动距离，移动距离{dist}，剩余{rest_movement}");
            return false;
        }
        ...
        CLogManager.LogInfo($"{Name}移动到了({Pos.x},{Pos.z})，移动距离{dist}，剩余{rest_movement-dist}");
    ...
    public bool MakeShadow(Vector3Int target)
    {
        ...
        // 创建一个新的GameObject，只包含SpriteRenderer组件
    ...
    public void Attack(CCharacter obj, EMove slot)
    {
        ...
        {
            CLogManager.LogInfo($"{Name}对不存在的对象使用了招式！");
            return;
        }
        if (m_moveRepo == null)
        {
            CLogManager.LogError($"{Name}没有m_moveRepo！");
            return;
        }
        IMove move;
        if (!m_moveRepo.GetMove(slot, out move))
        {
            CLogManager.LogInfo($"{Name}使用了不存在的招式{slot}！");
            return;
        }
